...
KingbaseES supports sequences, and SQLAlchemy uses these as the default means
...
place if KingbaseES 8.2 or later is in use. As a fallback approach, the
...
KingbaseES 10 IDENTITY columns
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

KingbaseES 10 has a new IDENTITY feature that supersedes the use of SERIAL.
...
IDENTITY::
    ...
    @compiles(CreateColumn, 'kingbase')
...
.. _kingbase_isolation_level:

Transaction Isolation Level
---------------------------

All KingbaseES dialects support setting of transaction isolation level
...
When using a non-ksycopg2 dialect, this feature works by issuing the command
...
* ``REPEATABLE READ``
* ``SERIALIZABLE``
* ``AUTOCOMMIT`` - on ksycopg2 only

.. seealso::

    :ref:`ksycopg2_isolation_level`

.. _kingbase_schema_reflection:

Remote-Schema Table Introspection and KingbaseES search_path
------------------------------------------------------------
...
The KingbaseES dialect can reflect tables from any schema.  The
...
By default, the KingbaseES dialect mimics the behavior encouraged by
KingbaseES's own ``pg_get_constraintdef()`` builtin procedure.  This function
returns a sample definition for a particular foreign key constraint,
omitting the referenced schema name from that definition when the name is
also in the KingbaseES schema search path.  The interaction below
...
reflection process as follows::
    ...
    >>> engine = create_engine("kingbase://scott:tiger@localhost/test")
...
``kingbase_ignore_search_path`` option, which can be specified as a
...
:meth:`_schema.MetaData.reflect`::
    ...
    ...                       autoload_with=conn,
    ...                       kingbase_ignore_search_path=True)
    ...
...
.. sidebar:: Best Practices for KingbaseES Schema reflection

    The description of KingbaseES schema reflection behavior is complex, and
...
``None``. The "default" schema on KingbaseES is that which is returned by the
KingbaseES ``current_schema()`` function.  On a typical KingbaseES
...
.. versionadded:: 0.9.2 Added the ``kingbase_ignore_search_path``
...
The dialect supports ``INSERT..RETURNING``, ``UPDATE..RETURNING`` and
...
.. _kingbase_insert_on_conflict:

INSERT...ON CONFLICT (Upsert)
------------------------------

KingbaseES allows "upserts" (update or insert) of
...
SQLAlchemy provides ``ON CONFLICT`` support via the KingbaseES-specific
:func:`_kingbase.insert()` function, which provides
the generative methods :meth:`~.kingbase.Insert.on_conflict_do_update`
and :meth:`~.kingbase.Insert.on_conflict_do_nothing`::

    from sqlalchemy.dialects.kingbase import insert
...
:attr:`~.kingbase.Insert.excluded` is available as an attribute on
the :class:`_kingbase.Insert` object; this object is a
...
:meth:`~.kingbase.Insert.on_conflict_do_nothing` method::
...
.. versionadded:: 1.1 Added support for KingbaseES ON CONFLICT clauses
...
SQLAlchemy makes available the KingbaseES ``@@`` operator via the
:meth:`_expression.ColumnElement.match`
method on any textual column expression.
On a KingbaseES dialect, an expression like the following::
...
The KingbaseES text search functions such as ``to_tsquery()``
...
The :class:`_kingbase.TSVECTOR` type can provide for explicit CAST::

    from sqlalchemy.dialects.kingbase import TSVECTOR
...
Full Text Searches in KingbaseES are influenced by a combination of: the
KingbaseES setting of ``default_text_search_config``, the ``regconfig`` used
...
searches) one may need to explicitly pass in a particular KingbaseES
...
search strategies, the ``match`` method accepts a ``kingbase_regconfig``
keyword argument::

    select([mytable.c.id]).where(
        mytable.c.title.match('somestring', kingbase_regconfig='english')
    )
...
``to_tsvector()`` command as the initial argument::
    ...
    select([mytable.c.id]).where(
            ...
            .match('somestring', kingbase_regconfig='english')
        )
...
KingbaseES to ensure that you are generating queries with SQLAlchemy that
take full advantage of any indexes you may have created for full text search.

FROM ONLY ...
------------------------

The dialect supports KingbaseES's ONLY keyword for targeting only a particular
table in an inheritance hierarchy. This can be used to produce the
``SELECT ... FROM ONLY``, ``UPDATE ONLY ...``, and ``DELETE FROM ONLY ...``
syntaxes. It uses SQLAlchemy's hints mechanism::

    # SELECT ... FROM ONLY ...
    result = table.select().with_hint(table, 'ONLY', 'kingbase')
    print(result.fetchall())
    ...
    # DELETE FROM ONLY ...
    table.delete().with_hint('ONLY', dialect_name='kingbase')


.. _kingbase_indexes:

KingbaseES-Specific Index Options
---------------------------------

Several extensions to the :class:`.Index` construct are available, specific
to the KingbaseES dialect.

.. _kingbase_partial_indexes:
...
using the ``kingbase_where`` keyword argument::

  Index('my_index', my_table.c.id, kingbase_where=my_table.c.value > 10)

Operator Classes
^^^^^^^^^^^^^^^^

KingbaseES allows the specification of an *operator class* for each column of
an index
The :class:`.Index` construct allows these to be specified via the
``kingbase_ops`` keyword argument::
...
Note that the keys in the ``kingbase_ops`` dictionary are the "key" name of
...
If ``kingbase_ops`` is to be used against a complex SQL expression such
...
KingbaseES provides several index types: B-Tree, Hash, GiST, and GIN, as well
as the ability for users to create their own. These can be
specified on :class:`.Index` using the ``kingbase_using`` keyword argument::

    Index('my_index', my_table.c.data, kingbase_using='gin')

The value passed to the keyword argument will be simply passed through to the
underlying CREATE INDEX command, so it *must* be a valid index type for your
version of KingbaseES.

.. _kingbase_index_storage:

Index Storage Parameters
^^^^^^^^^^^^^^^^^^^^^^^^

KingbaseES allows storage parameters to be set on indexes. The storage
parameters available depend on the index method used by the index. Storage
parameters can be specified on :class:`.Index` using the ``kingbase_with``
keyword argument::

    Index('my_index', my_table.c.data, kingbase_with={"fillfactor": 50})

.. versionadded:: 1.0.6

KingbaseES allows to define the tablespace in which to create the index.
The tablespace can be specified on :class:`.Index` using the
``kingbase_tablespace`` keyword argument::

    Index('my_index', my_table.c.data, kingbase_tablespace='my_tablespace')

.. versionadded:: 1.1

Note that the same option is available on :class:`_schema.Table` as well.

.. _kingbase_index_concurrently:

Indexes with CONCURRENTLY
^^^^^^^^^^^^^^^^^^^^^^^^^

The KingbaseES index option CONCURRENTLY is supported by passing the
flag ``kingbase_concurrently`` to the :class:`.Index` construct::

    tbl = Table('testtbl', m, Column('data', Integer))

    idx1 = Index('test_idx1', tbl.c.data, kingbase_concurrently=True)

The above index construct will render DDL for CREATE INDEX, assuming
KingbaseES or higher is detected or for a connection-less dialect, as::

    CREATE INDEX CONCURRENTLY test_idx1 ON testtbl (data)

For DROP INDEX, assuming KingbaseES is detected or for
...
.. versionadded:: 1.1 support for CONCURRENTLY on DROP INDEX.  The
   ...
   of KingbaseES is detected on the connection (or for a connection-less
   dialect).

When using CONCURRENTLY, the KingbaseES database requires that the statement
...
construct, the DBAPI's "autocommit" mode must be used::
    ...
    table = Table(
        ...
        Column("id", String))
    index = Index(
        "foo_idx", table.c.id, kingbase_concurrently=True)
...
.. seealso::
    ...
    :ref:`kingbase_isolation_level`

.. _kingbase_index_reflection:

KingbaseES Index Reflection
---------------------------

The KingbaseES database creates a UNIQUE INDEX implicitly whenever the
...
.. versionchanged:: 1.0.0 - :class:`_schema.Table` reflection now includes
   ...
   collection; the KingbaseES backend will no longer include a "mirrored"
...
used for the KingbaseES backend is an instance
of :class:`.PGInspector`, which offers additional methods::

    from sqlalchemy import create_engine, inspect

    engine = create_engine("kingbase+ksycopg2://localhost/test")
...
.. _kingbase_table_options:

KingbaseES Table Options
------------------------

Several options for CREATE TABLE are supported directly by the KingbaseES
dialect in conjunction with the :class:`_schema.Table` construct:

* ``TABLESPACE``::

    Table("some_table", metadata, ..., kingbase_tablespace='some_tablespace')

  The above option is also available on the :class:`.Index` construct.

* ``ON COMMIT``::

    Table("some_table", metadata, ..., kingbase_on_commit='PRESERVE ROWS')

* ``WITH OIDS``::

    Table("some_table", metadata, ..., kingbase_with_oids=True)

* ``WITHOUT OIDS``::

    Table("some_table", metadata, ..., kingbase_with_oids=False)

* ``INHERITS``::

    Table("some_table", metadata, ..., kingbase_inherits="some_supertable")

    Table("some_table", metadata, ..., kingbase_inherits=("t1", "t2", ...))
...
* ``PARTITION BY``::
    ...
    Table("some_table", metadata, ...,
          kingbase_partition_by='LIST (part_column)')
...
The KingbaseES dialect supports arrays, both as multidimensional column types
as well as array literals:

* :class:`_kingbase.ARRAY` - ARRAY datatype

* :class:`_kingbase.array` - array literal

* :func:`_kingbase.array_agg` - ARRAY_AGG SQL function

* :class:`_kingbase.aggregate_order_by` - helper for PG's ORDER BY aggregate
  function syntax.

JSON Types
----------

The KingbaseES dialect supports both JSON and JSONB datatypes, including
ksycopg2's native support and support for all of KingbaseES's special
operators:

* :class:`_kingbase.JSON`

* :class:`_kingbase.JSONB`

HSTORE Type
-----------

The KingbaseES HSTORE type as well as hstore literals are supported:

* :class:`_kingbase.HSTORE` - HSTORE datatype

* :class:`_kingbase.hstore` - hstore literal

ENUM Types
----------

KingbaseES has an independently creatable TYPE structure which is used
...
* :class:`_kingbase.ENUM` - DDL and typing support for ENUM.

* :meth:`.PGInspector.get_enums` - retrieve a listing of current ENUM types

* :meth:`.kingbase.ENUM.create` , :meth:`.kingbase.ENUM.drop` - individual
  CREATE and DROP commands for ENUM.

.. _kingbase_array_of_enum:
...
.. sourcecode:: python
    ...
    from sqlalchemy.dialects.kingbase import ARRAY
...
class MONEY(sqltypes.TypeEngine):
    ...
    """Provide the KingbaseES MONEY type.
...
class OID(sqltypes.TypeEngine):
    ...
    """Provide the KingbaseES OID type.
...
class REGCLASS(sqltypes.TypeEngine):
    ...
    """Provide the KingbaseES REGCLASS type.
...
class INTERVAL(sqltypes.NativeForEmulated, sqltypes._AbstractInterval):
    ...
    """KingbaseES INTERVAL type.

    The INTERVAL type may not be supported on all DBAPIs.
    It is known to work on ksycopg2 and not pg8000 or zxjdbc.
...
class UUID(sqltypes.TypeEngine):
    ...
    It is known to work on ksycopg2 and not pg8000.
...
class TSVECTOR(sqltypes.TypeEngine):
    ...
    """The :class:`_kingbase.TSVECTOR` type implements the KingbaseES
...
class ENUM(sqltypes.NativeForEmulated, sqltypes.Enum):
    ...
    True, the KingbaseES backend will use a :class:`_kingbase.ENUM`
    ...
    When using :class:`_types.Enum` or :class:`_kingbase.ENUM`
    ...
    :class:`_kingbase.ENUM` independently, and associate it with the
    ...
    .. versionchanged:: 1.0.0 The KingbaseES :class:`_kingbase.ENUM` type
    ...
    def __init__(self, *enums, **kw):
        """Construct an :class:`_kingbase.ENUM`.
        ...
        :param create_type: Defaults to True.
         ...
         :meth:`~.kingbase.ENUM.create`
         or :meth:`~.kingbase.ENUM.drop`
         are called directly.
         Setting to ``False`` is helpful
         when invoking a creation scheme to a SQL file
         without access to the actual database -
         the :meth:`~.kingbase.ENUM.create` and
         :meth:`~.kingbase.ENUM.drop` methods can
    ...
    def adapt_emulated_to_native(cls, impl, **kw):
        """Produce a KingbaseES native :class:`_kingbase.ENUM` from plain
        :class:`.Enum`.
    ...
    def create(self, bind=None, checkfirst=True):
        ...
        :class:`_kingbase.ENUM`.

        If the underlying dialect does not support
        KingbaseES CREATE TYPE, no action is taken.

        :param bind: a connectable :class:`_engine.Engine`,
         :class:`_engine.Connection`, or similar object to emit
         SQL.
        :param checkfirst: if ``True``, a query against
         the catalog will be first performed to see
    ...
    def drop(self, bind=None, checkfirst=True):
        ...
        If the underlying dialect does not support
        KingbaseES DROP TYPE, no action is taken.
...
class PGCompiler(compiler.SQLCompiler):
    ...
    def visit_match_op_binary(self, binary, operator, **kw):
        if "kingbase_regconfig" in binary.modifiers:
            regconfig = self.render_literal_value(
                binary.modifiers["kingbase_regconfig"], sqltypes.STRINGTYPE
            )
            if regconfig:
    ...
    def delete_extra_from_clause(
        ...
        """Render the DELETE .. USING clause specific to KingbaseES."""
        return "USING " + ", ".join(
            t._compiler_dispatch(self, asfrom=True, fromhints=from_hints, **kw)
            for t in extra_froms
        )
...
class PGDDLCompiler(compiler.DDLCompiler):
        ...
        if constraint._type_bound:
            ...
            if (
                ...
                raise exc.CompileError(
                    "KingbaseES dialect cannot produce the CHECK constraint "
    ...
    def visit_create_index(self, create):
        ...
        if self.dialect._supports_create_index_concurrently:
            concurrently = index.dialect_options["kingbase"]["concurrently"]
            if concurrently:
                text += "CONCURRENTLY "
        ...
        using = index.dialect_options["kingbase"]["using"]
        if using:
            text += (
                "USING %s "
        ...
        ops = index.dialect_options["kingbase"]["ops"]
        text += "(%s)" % (
            ", ".join(
                [
        ...
        withclause = index.dialect_options["kingbase"]["with"]
        ...
        tablespace_name = index.dialect_options["kingbase"]["tablespace"]

        if tablespace_name:
            text += " TABLESPACE %s" % preparer.quote(tablespace_name)

        whereclause = index.dialect_options["kingbase"]["where"]
    ...
    def post_create_table(self, table):
        table_opts = []
        pg_opts = table.dialect_options["kingbase"]
    ...
    def visit_computed_column(self, generated):
        if generated.persisted is False:
            raise exc.CompileError(
                "KingbaseES computed columns do not support 'virtual' "
...
class PGTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_enum(self, type_, **kw):
        if not type_.native_enum or not self.dialect.supports_native_enum:
            ...
            return self.visit_enum_identifier(type_, **kw)

    def visit_enum_identifier(self, type_, identifier_preparer=None, **kw):
...
class PGIdentifierPreparer(compiler.IdentifierPreparer):
    ...
    def format_type(self, type_, use_schema=True):
        if not type_.name:
            raise exc.CompileError("KingbaseES ENUM type requires a name.")
    ...
    def get_insert_default(self, column):
        if column.primary_key and column is column.table._autoincrement_column:
            if column.server_default and column.server_default.has_argument:
                ...
                try:
                    seq_name = column._kingbase_seq_name
                    ...
                    column._kingbase_seq_name = seq_name = name
...
class KBDialect(default.DefaultDialect):
    name = "kingbase"
    supports_alter = True
    ...
    reflection_options = ("kingbase_ignore_search_path",)
    ...
    def initialize(self, connection):
        super(KBDialect, self).initialize(connection)
        ...
        if not self.supports_native_enum:
            ...
            # ksycopg2, others may have placed ENUM here as well
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        if self.server_version_info < (8, 4):
            ...
            # unnest() and generate_subscripts() both introduced
            PK_SQL = """
                SELECT a.attname
    ...
    def get_foreign_keys(
        ...
        table_name,
        schema=None,
        kingbase_ignore_search_path=False,
        **kw
    ):
        ...
        for conname, condef, conschema in c.fetchall():
            ...
            if kingbase_ignore_search_path:
    ...
    def get_indexes(self, connection, table_name, schema, **kw):
        ...
        # returned as a list by some drivers such as pykingbase
        ...
        for row in c.fetchall():
            ...
            if not has_idx:
                ...
                # to have a ``kingbase_using`` in it that is just the
