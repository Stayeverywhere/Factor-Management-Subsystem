...
class JSON(sqltypes.JSON):
    """Represent the KingbaseES JSON type.
    ...
    .. versionchanged:: 1.1 :class:`_kingbase.JSON` is now a KingbaseES-
       specific specialization of the new :class:`_types.JSON` type.

    The operators provided by the KingbaseES version of :class:`_types.JSON`
    include:
    ...
    * Path index operations returning text (the ``#>>`` operator)::
    ...
    using ksycopg2, the DBAPI only allows serializers at the per-cursor
    ...
    When using the ksycopg2 dialect, the json_deserializer is registered
    against the database using ``ksycopg2.extras.register_default_json``.
...
class JSONB(JSON):
    """Represent the KingbaseES JSONB type.

    The :class:`_kingbase.JSONB` type stores arbitrary JSONB format data, e.
    g.::
    ...
    The :class:`_kingbase.JSONB` type includes all operations provided by
    ...
    Like the :class:`_types.JSON` type, the :class:`_kingbase.JSONB`
    ...
    ksycopg2, the serializers are associated with the jsonb type using
    ``ksycopg2.extras.register_default_jsonb`` on a per-connection basis,
    in the same way that ``ksycopg2.extras.register_default_json`` is used
