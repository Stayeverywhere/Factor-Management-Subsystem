...
DERIVED_FACTORS = [
    ...
    ('df-daily-ret',    'daily_ret',    '日收益率',    'ret = close_price / shift(close_price, 1) - 1', 'close_price'),
    ('df-ret-5',        'ret_5',        '5日累计收益率', 'ret_5 = close_price / shift(close_price, 5) - 1', 'close_price'),
    ('df-ret-20',       'ret_20',       '20日累计收益率', 'ret_20 = close_price / shift(close_price, 20) - 1', 'close_price'),
    ('df-ret-60',       'ret_60',       '60日累计收益率', 'ret_60 = close_price / shift(close_price, 60) - 1', 'close_price'),
    ('df-ma-5',          'ma_5',        '5日移动均价',   'ma_5 = mean(close_price, 5)', 'close_price'),
    ('df-ma-10',         'ma_10',       '10日移动均价',  'ma_10 = mean(close_price, 10)', 'close_price'),
    ('df-ma-20',         'ma_20',       '20日移动均价',  'ma_20 = mean(close_price, 20)', 'close_price'),
    ('df-ma-60',         'ma_60',       '60日移动均价',  'ma_60 = mean(close_price, 60)', 'close_price'),
    ('df-amp-5',         'amp_5',       '5日价格振幅',   'amp_5 = (max(high_price,5)-min(low_price,5))/shift(close_price,5)', 'high_price,low_price,close_price'),
    ('df-amp-20',        'amp_20',      '20日价格振幅',  'amp_20 = (max(high_price,20)-min(low_price,20))/shift(close_price,20)', 'high_price,low_price,close_price'),
    ...
    ('df-amount-avg-5',  'amount_avg_5','5日均成交额',   'amount_avg_5 = mean(turnover, 5)', 'turnover'),
    ('df-amount-avg-20', 'amount_avg_20','20日均成交额', 'amount_avg_20 = mean(turnover, 20)', 'turnover'),
    ('df-mom-20',        'mom_20',      '20日动量因子',  'mom_20 = close_price / shift(close_price, 20) - 1', 'close_price'),
    ('df-mom-60',        'mom_60',      '60日动量因子',  'mom_60 = close_price / shift(close_price, 60) - 1', 'close_price'),
    ('df-mom-120',       'mom_120',     '120日动量因子', 'mom_120 = close_price / shift(close_price, 120) - 1', 'close_price'),
    ('df-mom20-skip5',   'mom20_skip5', '标准动量(剔除1周)', 'mom20_skip5 = close_price / shift(close_price, 25) - 1', 'close_price'),
    ('df-rev-5',         'rev_5',       '5日反转因子',   'rev_5 = -(close_price / shift(close_price, 5) - 1)', 'close_price'),
    ('df-rsi-14',        'rsi_14',      'RSI相对强弱(14日)', 'RSI = 100 - 100/(1 + avg_gain/avg_loss)', 'close_price'),
    ...
    ('df-mdd-20',        'mdd_20',      '20日最大回撤',  'mdd_20 = 1 - max(close_price, 20) / min(close_price, 20)', 'close_price'),
    ('df-mdd-60',        'mdd_60',      '60日最大回撤',  'mdd_60 = 1 - max(close_price, 60) / min(close_price, 60)', 'close_price'),
    ('df-amihud',        'amihud',      'Amihud非流动性', 'amihud = abs(daily_ret) / turnover', 'daily_ret,turnover'),
]
