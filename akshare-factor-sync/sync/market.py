...
TABLE_NAME = 'ak_market_quote'
TASK_CODE = 'market_quote'


def sync_market_quote() -> int:
    ensure_core_schema()
    ...
    for _, row in df.iterrows():
        ...
        if not trade_date:
            continue
        payload = row.to_dict()
        save_raw_payload(TASK_CODE, f"stock_zh_a_hist:000001:{trade_date}", payload)
        rows.append({
            'market_type': 'stock_a',
            'symbol': '000001',
    ...
    with ENGINE.begin() as conn:
        for r in rows:
            conn.execute(text('''
                INSERT INTO ak_market_quote (market_type, symbol, trade_date, open_price, close_price, high_price, low_price, volume, turnover, source_system, created_at, updated_at)
                VALUES (:market_type, :symbol, :trade_date, :open_price, :close_price, :high_price, :low_price, :volume, :turnover, :source_system, :created_at, :updated_at)
                ON CONFLICT (market_type, symbol, trade_date) DO UPDATE SET
